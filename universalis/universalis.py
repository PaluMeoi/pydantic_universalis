...
class Universalis:
    ...
    def items(
        ...
        if len(itemIds) <= 100:
            ...
            if len(itemIds) > 1:
                ...
                return self.make_multi(MultiCurrent, response)
        else:
    ...
    def make_multi(self, model, response):
        current = Current.model_validate(response)
        return model.model_validate(
            {
                "itemIDs":         [current.itemID],
                "items":           {current.itemID: current},
                "worldID":         current.worldID,
                "dcName":          current.dcName,
    ...
    def items_history(
        ...
        if len(itemIds) <= 100:
            response = self.request_items_history(
                worldDcRegion,
                itemIds,
                entriesToReturn=entriesToReturn,
                statsWithin=statsWithin,
            ...
            if len(itemIds) > 1:
                ...
                # Changing the request into a "fake" MultiCurrent if only one item is requested
                return self.make_multi(MultiHistory, response)
        else:
...
if __name__ == "__main__":
    ...
    data = uni.item(
        "Excalibur", 5488, fields=("listings.pricePerUnit", "listings.quantity")
    )
    print(data)
